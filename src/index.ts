...
export class ProxySettings {
  readonly certificatesFolder = process.env.PROXY_CERTS_FOLDER;
  readonly certificateFile = "fullchain.pem";
  readonly keyFile = "privkey.pem";
  readonly httpPort = 80;
  readonly httpsPort = 443;
...
export class ProxyServer extends EventEmitter {
  ...
  protected proxies: Record<string, ProxyEntry> = {};
  ...
  start() {
    ...
    this.servers = [
      createHttpServer((req, res) => this.serveRequest(req, res, true)).listen(
        this.settings.httpPort
      ),

      createHttpsServer(this.getSslOptions(), (req, res) =>
        this.serveRequest(req, res, false)
  ...
  protected async loadCertificate(folder: string) {
    ...
    createSecureContext({
      cert: await readFile(
        join(certificatesFolder, folder, certificateFile),
        "utf8"
      ),
  ...
  protected serveRequest(
    ...
    proxyRequest.setHeader(
      "forwarded",
      "host=" + req.headers.host + ";proto=" + (insecure ? "http" : "https")
    );
    ...
    proxyRequest.on("response", (proxyRes) => {
      ...
      const isCorsSimple =
        req.method !== "OPTIONS" && proxyEntry.cors && req.headers.origin;
      if (isCorsSimple) {
        this.setCorsHeaders(req, res);
      }
  ...
  protected setHeaders(
    ...
    to: ServerResponse | ClientRequest
  ) {
